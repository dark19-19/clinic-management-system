@extends('layouts.app')
@section('title','Users Appointments')
@section('page.title','Users Appointments')
@section('content')
    <div class="card" style="min-height: 62vh">
        <div class="title">
            <h4>Users Appointments To Be Checked</h4>
        </div>
        <div class="content">
            <table>
                <thead>
                ...
                @foreach($appointments as $appointment)
                    <tr>
                        ...
                        <td class="quick-actions flex flex-row">
                            <a href="{{route('user.appointment.accept', $appointment->id)}}" style="fill: var(--success); color: var(--success); font-size: 11px">
                            ...
                            <form id="delete-form-{{$appointment->id}}" action="{{route('user.appointment.delete', $appointment->id)}}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            <form id="reject-form-{{$appointment->id}}" action="{{route('user.appointment.reject', $appointment->id)}}" method="POST" style="display: none;">
                                @csrf
                                @method('PATCH')
                            </form>
                        </td>
